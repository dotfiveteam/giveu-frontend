<template>
  <div>
    ...
    <Nanum v-if="nanum_toggle_exclusive === 0" v-bind:videos="recently_added_list" />
...
export default {
  ...
  beforeMount() {
    this.getVideos();
    console.log("Home.vue beforeMount(): " + this.date_sorted_list.length);
  },
  methods: {
    getVideos() {
      // get the list from API server -> channel_list
      // sort videos in the order of published date -> date_sorted_list
      // extract newly added (past 7 days) videos -> recently_added_list
      this.axios
        ...
        .then(response => {
          ...
          for (let i = 0; i < this.date_sorted_list.length; i++) {
            if (this.date_sorted_list[i].published_at > today) {
              this.recently_added_list.push(this.date_sorted_list[i]);
            } else {
              break;
            }
          }
